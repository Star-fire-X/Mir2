...
server::GameConfig MakeCooldownConfig() {
  server::GameConfig config = MakeValidConfig();
  config.monster_spawns.push_back(
      server::MonsterSpawn{.monster_template_id = 2001});
  return config;
}

std::vector<protocol::ClientMessage> WaitForInbound(
    NetworkManager* network_manager, std::size_t expected_count) {
  ...
  for (int attempt = 0; attempt < 20; ++attempt) {
    ...
    inbound_messages.insert(inbound_messages.end(), inbound.begin(),
                            inbound.end());
...
TEST(NetworkRuntimeTest, QueueOutboundDeliversTcpLoginAndEnterSceneResponses) {
  ...
  ASSERT_TRUE(std::holds_alternative<protocol::LoginResponseMessage>(
      login_messages[0]));
  ...
  network_manager.QueueOutbound(
      protocol::OutboundMessage{shared::EnterSceneRequest{login.player_id, 1}});
...
TEST(NetworkRuntimeTest, KcpSceneChannelCarriesMoveCombatAndPickupFlow) {
  ...
  network_manager.QueueOutbound(
      protocol::OutboundMessage{shared::CastSkillRequest{
...
TEST(NetworkRuntimeTest, CastSkillUsesMonotonicClockForCooldowns) {
  std::atomic<std::uint64_t> now_ms{0};
  server::ConfigManager config_manager;
  config_manager.Load(MakeCooldownConfig());
  server::ServerRuntime runtime(
      config_manager, server::ServerRuntime::Options{
                          .tcp_port = 0,
                          .udp_port = 0,
                          .now_ms = [&now_ms]() { return now_ms.load(); },
                      });
  ASSERT_TRUE(runtime.Start());
  ...
  const shared::EnterSceneSnapshot snapshot =
      std::get<protocol::EnterSceneSnapshotMessage>(enter_messages[1]).snapshot;
  const auto visible_monster_it = std::find_if(
      snapshot.visible_entities.begin(), snapshot.visible_entities.end(),
      [](const shared::VisibleEntitySnapshot& entity) {
        return entity.kind == shared::VisibleEntityKind::kMonster;
      });
  ASSERT_NE(visible_monster_it, snapshot.visible_entities.end());
  ...
  const std::vector<protocol::ClientMessage> move_messages =
      WaitForInbound(&network_manager, 2U);
  ASSERT_EQ(move_messages.size(), 2U);
  const auto entered_monster_it = std::find_if(
      move_messages.begin(), move_messages.end(),
      [](const protocol::ClientMessage& message) {
        return std::holds_alternative<protocol::AoiEnterMessage>(message) &&
               std::get<protocol::AoiEnterMessage>(message).event.entity.kind ==
                   shared::VisibleEntityKind::kMonster;
      });
  ASSERT_NE(entered_monster_it, move_messages.end());
  const shared::VisibleEntitySnapshot entered_monster =
      std::get<protocol::AoiEnterMessage>(*entered_monster_it).event.entity;

  network_manager.QueueOutbound(protocol::OutboundMessage{
      shared::MoveRequest{snapshot.controlled_entity_id,
                          shared::ScenePosition{8.0F, 4.0F}, 2, 1001}});
  const std::vector<protocol::ClientMessage> align_messages =
      WaitForInbound(&network_manager, 1U);
  ASSERT_EQ(align_messages.size(), 1U);
  ASSERT_TRUE(
      std::holds_alternative<protocol::SelfStateMessage>(align_messages[0]));

  network_manager.QueueOutbound(protocol::OutboundMessage{
      shared::CastSkillRequest{snapshot.controlled_entity_id,
                               visible_monster_it->entity_id, 1001, 3}});
  const std::vector<protocol::ClientMessage> first_cast_messages =
      WaitForInbound(&network_manager, 3U);
  ASSERT_EQ(first_cast_messages.size(), 3U);
  EXPECT_EQ(std::get<protocol::CastSkillResultMessage>(first_cast_messages[0])
                .result.error_code,
            shared::ErrorCode::kOk);

  network_manager.QueueOutbound(
      protocol::OutboundMessage{shared::CastSkillRequest{
          snapshot.controlled_entity_id, entered_monster.entity_id, 1001, 4}});
  const std::vector<protocol::ClientMessage> cooldown_messages =
      WaitForInbound(&network_manager, 1U);
  ASSERT_EQ(cooldown_messages.size(), 1U);
  ASSERT_TRUE(std::holds_alternative<protocol::CastSkillResultMessage>(
      cooldown_messages[0]));
  EXPECT_EQ(std::get<protocol::CastSkillResultMessage>(cooldown_messages[0])
                .result.error_code,
  ...
  network_manager.QueueOutbound(
      protocol::OutboundMessage{shared::CastSkillRequest{
          snapshot.controlled_entity_id, entered_monster.entity_id, 1001, 5}});
  const std::vector<protocol::ClientMessage> second_cast_messages =
      WaitForInbound(&network_manager, 3U);
  ASSERT_EQ(second_cast_messages.size(), 3U);
  ASSERT_TRUE(std::holds_alternative<protocol::CastSkillResultMessage>(
      second_cast_messages[0]));
  EXPECT_EQ(std::get<protocol::CastSkillResultMessage>(second_cast_messages[0])
                .result.error_code,
            shared::ErrorCode::kOk);
